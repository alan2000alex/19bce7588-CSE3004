public class CarFuel {
    ...
    int minRefill(int total_distance, int full_tank, int stops[], int no_of_stops) {
        ...
        while(total_distance > 0) {
            ...
            if(current_refill == last_refill || current_refill >= no_of_stops)
                return -1;
    ...
    public static void main(String args[]) {
        ...
        tot_dist = 1010; full_tank = 400; no_of_stops = 4;
        int stops[] = {275,375,550,750};
  
        CarFuel obj = new CarFuel();
        min_refills = obj.minRefill(tot_dist+(tot_dist - stops[no_of_stops-1]), full_tank, stops, no_of_stops);

        System.out.println("Minimum Refills required to travel distance d = "+(tot_dist+(tot_dist - stops[no_of_stops-1]))+"km : "+ min_refills);
    }
}
